...
local function finish_stopping_watcher(project, watcher, result, payload)
    ...
    if watcher and watcher.stop_callback then
        watcher.stop_callback(payload)
    end
end
...
function M.start(project, callback, run_config)
    ...
    local command, deps_path = compiler_command.build("watch", project, opts)
    compiler_service.set(project, {
    ...
    local watcher_operation = operation.run("compiler-typst-watch", command, {
        cwd = project.root,
        text = true,
        ...
        on_finish = function(result)
            finish_watcher(project, watcher_state, deps_path, result, callback)
        end,
    })
