...
| `runtime.hooks` | ordered reset-hook registry | static registry | n/a | n/a | `runtime_reset_hooks_spec.lua` |
...
| `core.log` | in-memory log entries and optional JSONL sink state | `log.clear()` / `cache_registry.reset()` | n/a | n/a | `log_spec.lua`, `bug_report_spec.lua` |
...
| `project.services.operations` | active project operation records | resource supervisor reset | project cancel/detach paths | n/a | lifecycle matrix, provider matrix |
...
| `core.operation` / `core.process` | active and retained process/operation handles | resource supervisor reset | project operation cancellation | n/a | compiler lifecycle and operation specs |
...
| `completion.context` | buffer-keyed completion context cache | `clear_cache()` via cache registry | `clear_cache(bufnr)` | n/a | `completion_context_cache_reset_spec.lua` |
| `completion.packages` | package metadata cache and prewarm state | package reset/cache registry | n/a | n/a | package/completion specs |
...
| `preview.source_maps.typst_query` | source-map query/cache state | cache registry reset | n/a | n/a | source-map specs |
