...
- `tinymist.profileCurrentFile`: pass-through command. typst.nvim's profile
  workflow remains plugin-owned so reports, provider state, and cancellation are
  consistent.
- Commands returned by `textDocument/codeAction`, `codeAction/resolve`,
  ...
  execute-command bridge. This includes export/test/profile style commands
  returned by Tinymist.
...
| Code lens | `textDocument/codeLens`, `codeLens/resolve`, code-lens commands | `vim.lsp.codelens.refresh()`, `vim.lsp.codelens.run()` | `tinymist.code_lens()`, `typst.semantic.code_lens()`, `:TypstCodeLens`, `tinymist.execute_command()` | WRAPPED | `tests/unit/tinymist_features_spec.lua`, `tests/unit/tinymist_execute_command_spec.lua`, `tests/unit/phase3_spec.lua` | Wrapper lists and executes selected lenses while native code-lens refresh remains available. Returned export/test/profile commands go through the shared execute-command helper. |
...
| Tests | `tinymist test` | None | `typst.development.test()`, `:TypstTest` | PLUGIN_OWNED | `tests/unit/phase3_spec.lua`, `tests/unit/provider_contract_spec.lua` | Workflow state, reporting, provider contracts, and cancellation are plugin-owned even when the default provider shells out to Tinymist. |
| Benchmarks | Tinymist/crityp depending on provider | None | `typst.development.bench()`, `:TypstBench` | PLUGIN_OWNED | `tests/unit/phase3_spec.lua` | Provider-driven workflow. Tinymist or crityp can supply the backend, but typst.nvim owns the command surface. |
| Coverage | `tinymist test --coverage` | None | `typst.development.coverage()`, `:TypstCoverage` | PLUGIN_OWNED | `tests/unit/phase3_spec.lua` | Default provider can route through Tinymist CLI; status/reporting remain plugin-owned. |
...
| Trace/profile current document | `tinymist.profileCurrentFile` | None | `typst.development.profile()`, `:TypstProfile`, `tinymist.execute_command()` | PLUGIN_OWNED | `tests/integration/commands/profile_spec.lua`, `tests/unit/phase3_spec.lua`, `tests/unit/tinymist_execute_command_spec.lua` | typst.nvim owns profile reporting. Tinymist's profile command is reachable through the execute-command helper when a native client is attached. |
