...
| `index` | `project.index_service`, `project.index_scheduler`, index helpers | headings, labels, definitions, bibliography, per-file index data, index generation | Mark dirty on source edits and refresh lazily. Cached reads should use generation checks. |
